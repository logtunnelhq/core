# Layered — Full Project Specification
...
Layered takes raw commit messages and company context as input and produces four
...
- CLI tool (`dotnet tool install -g layered`)
...
- Public changelog page (layered.so/changelog/product)
- Custom domains
...
│   │
│   └── Layered.Platform/        — paid platform features
...
# First time setup — saves .layered.json in repo root
layered configure
...
git log --oneline -20 | layered translate

# Translate and output to files
layered translate --input commits.txt --output ./changelogs/

# Translate specific audience only
layered translate --input commits.txt --audience ceo
```

Config file format (`.layered.json` in repo root):
...
*This document is the single source of truth for the Layered project.
