...
[![build](https://img.shields.io/github/actions/workflow/status/your-org/layered/ci.yml?branch=main&label=build)](https://github.com/your-org/layered/actions)
[![license](https://img.shields.io/badge/license-AGPL--3.0-blue)](LICENSE)
[![nuget](https://img.shields.io/nuget/v/Layered?label=nuget)](https://www.nuget.org/packages/Layered)

Layered translates raw Git commits into four audience-specific changelogs — one set of commits, four outputs (tech lead, manager, CEO, public-facing), generated in parallel by the LLM of your choice.
...
So a PM wanting to write a customer newsletter has three options: interrupt a developer, guess at what shipped, or skip the update. Existing tools (Headway, Beamer, Announcekit) solve the publishing step, not the translation step — they all assume someone has already turned commits into prose. Layered is the missing step. It takes the commits and produces four parallel versions, each tuned for a specific reader, using prompt templates you control.

## Quick start

```sh
# 1. Install the CLI
dotnet tool install -g Layered
...
# 3. Inside your repo, create a .layered.json
cd ~/code/your-product
layered configure

# 4. Translate
git log --oneline -20 > commits.txt
layered translate --input commits.txt
```
...
If you don't want the global tool, clone the repo and run `dotnet run --project src/Layered.Cli -- translate ...` instead. See [Self-hosting](#self-hosting) below.

## CLI reference

### `layered translate`

In-process translation. Reads `.layered.json`, calls the configured LLM for each audience in parallel, prints or writes the result.

| Flag | Short | Required | Default | Description |
|---|---|---|---|---|
| `--input` | `-i` | yes | — | File containing raw commit messages |
| `--config` | `-c` | no | `./.layered.json` | Path to the project config |
...
Interactive prompts for company context, then writes `.layered.json` to the current directory. No flags.

The file is seeded with sensible defaults for all four audiences — edit it later to tune tone, format, and per-audience instructions. If a `.layered.json` already exists, configure asks `[y/N]` before overwriting.
...
Persists a `.layered.json` on the server's disk. Useful for the hosted platform's onboarding flow; self-hosters more often write the file by hand or via `layered configure`.
...
```json
{
  ...
  "path": "/var/layered/.layered.json"
}
```

By default the file lands in the API's current working directory. Override with `Layered__Config__OutputDirectory=/some/path` (or the equivalent `appsettings.json` section).

## Self-hosting

```sh
git clone https://github.com/your-org/layered.git
cd layered
...
# Run the API
dotnet run --project src/Layered.Api

# Or run the CLI directly without installing the global tool
dotnet run --project src/Layered.Cli -- translate --input commits.txt
```
...
Adding a new provider is a switch case in `src/Layered.Core/Llm/LlmConnectorRegistrar.cs` plus a row in this table.
...
and a `.layered.json` describing a B2B invoicing tool for freelancers, you get four parallel outputs.
...
There's no CLA. By contributing you agree your changes are licensed under AGPL-3.0 along with the rest of the project. The `tests/Layered.Core.Tests` suite must stay green; run `dotnet test` from the repo root before pushing. If you're adding a new LLM provider, please also add it to the [provider table](#supported-llm-providers) in this README and to the connector switch in `src/Layered.Core/Llm/LlmConnectorRegistrar.cs`.
...
If you want to build a competing hosted product on top of Layered, the AGPL means your modifications need to be open-sourced too. If you just want to run it for your own team or your own customers, you can do whatever you like with it.
