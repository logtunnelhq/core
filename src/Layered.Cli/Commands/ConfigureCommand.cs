...
/// <summary>
/// <c>layered configure</c> — interactively prompt for the company
/// context fields, seed a sensible default audience set, and save the
/// result as <c>.layered.json</c> in the current working directory so
/// subsequent <c>layered translate</c> invocations pick it up.
/// </summary>
/// <remarks>
/// Configure intentionally does not require any LLM configuration: a
/// user can run <c>layered configure</c> on a fresh checkout before
...
public sealed class ConfigureCommand
{
    ...
    public ConfigureCommand(LayeredProjectConfigStore configStore)
    {
    ...
    public Command Build()
    {
        var command = new Command(
            "configure",
            "Interactively create a .layered.json file in the current directory.");
    ...
    private async Task<int> RunAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            var targetPath = Path.Combine(directory, LayeredProjectConfigStore.DefaultFileName);
            ...
            Console.WriteLine();
            Console.WriteLine("Layered project configuration");
            Console.WriteLine("=============================");
            Console.WriteLine();
            Console.WriteLine("Answer four short questions about your product. The CLI will write");
            Console.WriteLine("a .layered.json file you can edit later, with sensible defaults for");
            ...
            var config = new LayeredProjectConfig(
                Context: context,
