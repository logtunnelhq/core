...
/// <summary>
/// <c>logtunnel translate</c> — load <c>.logtunnel.json</c>, build a
...
/// <see cref="LlmConnectorRegistrar"/> so that <c>logtunnel configure</c>
...
public sealed class TranslateCommand
{
    ...
    public Command Build()
    {
        ...
        {
            Description = $"Path to {LogTunnelProjectConfigStore.DefaultFileName}. Defaults to the current working directory.",
            DefaultValueFactory = _ => new FileInfo(
                Path.Combine(Directory.GetCurrentDirectory(), LogTunnelProjectConfigStore.DefaultFileName)),
        };
